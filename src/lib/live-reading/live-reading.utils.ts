...
export function showNoLeaderToast() {
    toast.info("Nur der Leiter kann dies tun", {
        description: "Als Teilnehmer kannst du nicht zu einem anderen Vers springen. Diese Funktion ist exklusiv für den Leiter der Live-Lesung.",
        action: {
            label: "Mehr erfahren",
    ...
    channelA
        .on('presence', { event: 'sync' }, () => {
            ...
            if (key) {
                ...
                duaStore.update((state) => ({ ...state, currentVerse: newCurrentVerse }));
            }
        })
        .subscribe((status) => {
            if (status !== 'SUBSCRIBED') { return }
            console.log("Subscribed to broadcast channel " + code)
            liveReadingStore.update((state) => ({ ...state, isLiveReading: true, liveReadingCode: code }));
        })
    ...
    channelB.subscribe((status) => {
        ...
        duaStore.subscribe(duaState => {
            channelB.track({ currentVerse: duaState.currentVerse }).then(presenceTrackStatus => {
                console.log("Presence Message gesendet:" + duaState.currentVerse + " with response " + JSON.stringify(presenceTrackStatus));
            })
        })
    });
    liveReadingStore.update((state) => ({ 
