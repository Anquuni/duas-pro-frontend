...
<div
  ...
  style:transform={isHidden
    ? `translateY(calc(${getHeight(5)}px + env(safe-area-inset-bottom, 0px)))`
    : "translateY(0)"}>
  <div
    class="audio-player relative bg-background"
    style="padding-bottom: calc(0px + env(safe-area-inset-bottom, 0px));">
    <!-- Toggle Button -->
    <button
      ...
      style={`bottom: calc(${getHeight(10)}px + env(safe-area-inset-bottom, 0px));`}
