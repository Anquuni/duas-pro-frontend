<script lang="ts">
  ...
  let {
    ...
    data: { duas: any[]; pagination: { totalPages: number; totalCount: number } };
    titleKey?: string;
  ...
  function removeTypeFilter(typeToRemove: string) {
    const params = new URLSearchParams(page.url.searchParams);
    const remaining = params.getAll("types").filter((ty) => ty !== typeToRemove);
    params.delete("types");
    remaining.forEach((ty) => params.append("types", ty));
    params.set("page", "1");
    goto(`${page.url.pathname}?${params.toString()}`, { replaceState: true });
  }
</script>

<SeoHead title={$t(titleKey)} description={$t(descriptionKey)} type="organization" />
...
<div class="p-4 sm:p-6 md:p-8">
  ...
  {#if selectedTypes.filter((ty) => ty !== fixedType).length > 0}
    <div class="mb-4 mt-4 flex flex-wrap gap-2">
      {#each selectedTypes.filter((ty) => ty !== fixedType) as type}
        <div class="flex items-center gap-2 rounded-full border border-primary bg-primary/10 px-3 py-1 text-sm text-primary">
          <span>Typ: {type}</span>
  ...
  <DuaGrid duas={data.duas} {basePath} />
