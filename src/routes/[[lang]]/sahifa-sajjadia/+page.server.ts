...
export async function load({ params, url }) {
  ...
  let requestUrl = `duas?languages=${languages}&page=${page}&size=${size}&collections=sahifa-sajjadia`;
  if (searchWord) {
  ...
  if (errorResponse) {
    console.error(`Error while fetching Sahifa Sajjadia duas from ${requestUrl}: ${errorResponse}`);
    error(404);
  }
