...
export async function load({ params, url }) {
  ...
  const { data: response, error: errorResponse } = await supabase.functions.invoke(
    `duas/${params.ziyaratRouteName}?languages=${languages}`
  );
  if (errorResponse) {
    error(404);
  }
  ...
  return {
    dua,
    duaSlug: params.ziyaratRouteName,
    lang: systemLang,
  };
}
